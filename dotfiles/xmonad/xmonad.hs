...
import XMonad.Prompt hiding (border)
...
main :: IO ()
main = do
  xmproc   <- spawnPipe "xmobar --dock ~/.xmonad/xmobar.config"
  xmonad
    ...
    { manageHook =
        composeAll
          ...
          , ( className =? "Xloadimage"
              <&&> title =? "/home/handre/moonlander/moonlander-key-layout-combined.png"
            ) --> (placeHook (inBounds(fixed (0.5, 1))) <> doFloat)
          , manageHook cfg
          ]
    }
   where
     ...
     moveIt _ =
      Just
        <$> doFloatDep
              (\(RationalRect l t w h) ->
                let border' = 0.025
                    minSz = 0.0125
                    l' = max l border'
                    t' = max t border'
                    w' = min (w * 1.15) (max (1.0 - border' - l') minSz)
                    h' = min (h * 1.15) (max (1.0 - border' - t') minSz)
                in
                RationalRect l' t' w' h'
              )
...
myHooks :: Handle -> XConfig l -> XConfig l
myHooks xmproc cfg = cfg
  { startupHook =
      composeAll
        [ startup
        , startupHook cfg
        , setWMName "LG3D" -- Let JAVA GUI apps work https://hackage.haskell.org/package/xmonad-contrib-0.16/docs/XMonad-Hooks-SetWMName.html
        ]
...
renderWorkspace ::  String -> String -> String
...
startup :: X ()
startup = do
  trace "starting hello there"
  spawn "bash ~/.xmonad/startup.sh"
  trace "startup script run"
...
myLayoutHook =
  showWorkspaceName
    ...
    -- $ mirrorToggle
    -- $ reflectToggle
    $ avoidFloats tall ||| avoidFloats bsp ||| avoidFloats threeCol ||| avoidFloats grid
  where
   ...
   in -----------------------------------------------------------------------
      ...
      subKeys
        ...
        [ ("M-q", addName "Restart XMonad" $ spawn "xmonad --restart"),
          ...
          ("M-x", addName "Avoid float window" $ withFocused $ sendMessage . AvoidFloatToggleItem),
          ( "M-S-x"
          , addName
              "Avoid float window"
              (sendMessage AvoidFloatClearItems >> sendMessage (AvoidFloatSet False))
          ),
          ("M-<F4>", addName "Print Screen" $ return ()),
          ("M1-S-C-k", addName "show keymap" $ spawn "xloadimage /home/handre/moonlander/moonlander-key-layout-combined.png")
        ]
        ^++^
        -- -----------------------------------------------------------------------
        -- -- Launchers
        ...
        subKeys
          ...
          ( [ ("M-<Backspace>", addName "Kill" kill1),
              ...
              ("M-'", addName "Swap tab D" $ onGroup W.focusDown'),
              ("M-;", addName "Swap tab U" $ onGroup W.focusUp')
            ]
...
myFont = "xft:Roboto Mono for Powerline:size=8"

myBigFont = "xft:Roboto Mono for Powerline:size=15"

myWideFont = "xft:Roboto Mono for Powerline:size=30"
